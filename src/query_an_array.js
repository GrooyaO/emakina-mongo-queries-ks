...
(async () => {
  try {
    ...
    const serbianLanguage = await collection.find({
      languages: 'Serbian',
    }).count();
    ...
    const onlySerbianLanguage = await collection.find({
      languages: ['Serbian'],
    }).count();
    ...
    const serbianAndEnglish = await collection.find({
      languages: {
        $all: ['Serbian', 'English'],
      },
    }).count();
    ...
    const randomNumResult = await collection.find({
      someRandomNumbers: {
        $elemMatch: { $gt: 30, $lte: 40 },
      },
    }).count();

    console.log(`🎬 ${randomNumResult} movies, where some element of 'someRandomNumbers' is bigger than 30, and less or equal to 50.\n\n`);
  } catch (error) {
